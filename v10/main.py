...
class Show(Toplevel):
     def __init__(self, master=None, nome_planilha=""):
        ...
        vsb = Scrollbar(self.frame_planilha, orient="vertical", command=self.tree.yview)
        vsb.pack(side=RIGHT, fill='y')

        self.tree.configure(yscrollcommand=vsb.set)

        self.search_label = Label(self.frame_pesquisa, text="Procurar:")
        self.search_label["font"] = "Arial 12 bold"
        self.search_label.grid(row=0, column=0)

        self.search_entry = Entry(self.frame_pesquisa)
