...
class Waiting:
    def __init__(self, master=None):
        ...
        pro = ttk.Progressbar(
            self.firstContainer, orient=HORIZONTAL, mode="determinate", length=900)
    ...
    def task(self):
        ...
        root1.title("Cadastro automático WebValidade v10")
        Janela(root)
        root1.mainloop()
...
class Janela:
    def __init__(self, master=None):
        ...
        # Titulo
        self.titleLabel = Label(self.titleContainer,
                                text="Automação WebValidade")
        ...
        self.passwordLabel = Label(
            self.firstContainer, text="Senha para Login:")
        ...
        self.selectLabel = Label(self.secondContainer,
                                 text="Selecione a Planilha (.xlsx):")
        ...
        self.help = Button(self.thirdContainer, text="i",
                           bd=1, relief="ridge", width=3, height=1)
        ...
        self.registerButton = Button(
            self.fourthContainer, text="Cadastar Usuários")
    ...
    def show(self):
        try:
            ...
            messagebox.showerror(
                "Erro!", "Por favor, selecione o caminho da Planilha!")

        else:
            window = Show(self, nome_planilha=filename1)
            window.grab_set()
    ...
    def remember_data(self):
        ...
        if self.rememberVar.get() == 0:
            ...
            w = csv.writer(f)
            # 3. grava as linhas
            w.writerow([self.rememberVar.get(),
                       self.mailEntry.get(), self.passwordEntry.get()])
    ...
    def register(self):
        ...
        try:
            ...
            # Mostrar mensagem de erro caso a senha esteja em branco
                ...
                messagebox.showerror(
                    "Erro!", "Por favor, não deixe os campos em branco!")
            else:
                ...
                navegador.find_element_by_xpath(
                    '//*[@id="sign_in"]/div[3]/div/input').send_keys(email)
                navegador.find_element_by_xpath(
                    '//*[@id="sign_in"]/div[4]/div/input').send_keys(password)
                navegador.find_element_by_xpath(
                    '//*[@id="sign_in"]/div[5]/div[2]/button').click()
                sleep(5)
                ...
                sleep(5)
                navegador.find_element_by_xpath(
                    '//*[@id="form1"]/nav/ul/div[1]/a[4]/i').click()
                sleep(5)
                ...
                for nome, sobrenome, email1, senha in dados:
                    ...
                    navegador.find_element_by_xpath(
                        '//*[@id="wizard_with_validation"]/div[3]/ul/li[2]/a').click()
                    sleep(5)
                    ...
                    navegador.find_element_by_xpath(
                        '//*[@id="wizard_with_validation"]/div[3]/ul/li[3]/a').click()
                    sleep(5)
                ...
                messagebox.showinfo(
                    "Fim!", "Usuários cadastrados com sucesso!")
...
class Show(Toplevel):
    def __init__(self, master=None, nome_planilha=""):
        ...
        self.frame_planilha = Frame(self)
        self.frame_planilha.pack(side=LEFT)

        self.frame_pesquisa = LabelFrame(self, text="Filtros")
        self.frame_pesquisa.pack(side=RIGHT, fill="both", expand="yes")

        self.frame_pesquisa_textos = Frame(self.frame_pesquisa)
        self.frame_pesquisa_textos.grid(row=0, pady=50, padx=20)

        self.frame_pesquisa_botoes = Frame(self.frame_pesquisa)
        self.frame_pesquisa_botoes.grid(row=1)
        ...
        vsb = Scrollbar(self.frame_planilha, orient="vertical",
        ...
        hsb = Scrollbar(self.frame_planilha, orient="horizontal",
        ...
        self.search_label = Label(self.frame_pesquisa_textos, text="Procurar:")
        self.search_label["font"] = "Arial 12 bold"
        self.search_label.grid(row=0, column=0)

        self.search_entry = Entry(self.frame_pesquisa_textos)
        self.search_entry.grid(row=0, column=1)
        self.search_entry.bind("<KeyRelease>", self.search)

        self.filter_label = Label(self.frame_pesquisa_textos, text="Nome da coluna:")
        self.filter_label["font"] = "Arial 12 bold"
        self.filter_label.grid(row=1, column=0)

        self.filter_entry = Entry(self.frame_pesquisa_textos)
        self.filter_entry.grid(row=1, column=1)


        self.az_filter_button = Button(self.frame_pesquisa_botoes, text="A-Z", width=7, height=3)
        self.az_filter_button["command"] = lambda: self.azfilter(False)
        self.az_filter_button.grid(row=0, pady=15)

        self.za_filter_button = Button(self.frame_pesquisa_botoes, text="Z-A", width=7, height=3)
        self.za_filter_button["command"] = lambda: self.azfilter(True)
        self.za_filter_button.grid(row=1, pady=15)

        self.reset_filter_button = Button(self.frame_pesquisa_botoes, text="Resetar", width=7, height=3)
        self.reset_filter_button["command"] = self.resetfilter
        self.reset_filter_button.grid(row=2, pady=15)
    ...
    def azfilter(self, reverse):
        self.resetfilter()
        name = self.filter_entry.get()
        reverse = reverse
        l = [(self.tree.set(k, name), k) for k in self.tree.get_children('')]
        # print(self.tree.get_children(''))
        l.sort(reverse=reverse)  # Sort by
        # rearrange items in sorted positions
        for index, (val, k) in enumerate(l):  # Move according to the index after sorting
            self.tree.move(k, '', index)
            # print(k)
        if not reverse:
            self.tree.heading(name, text=f"{name} A-Z")
        elif reverse:
            self.tree.heading(name, text=f"{name} Z-A")
